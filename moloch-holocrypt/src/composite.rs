...
use arcanum_pqc::dsa::{MlDsa65, MlDsa65Signature, MlDsa65SigningKey, MlDsa65VerifyingKey};
use arcanum_pqc::PostQuantumSignature;
use arcanum_signatures::ed25519::{Ed25519Signature, Ed25519SigningKey, Ed25519VerifyingKey};
use arcanum_signatures::{
    Signature as SignatureTrait, SigningKey as SigningKeyTrait, VerifyingKey as VerifyingKeyTrait,
};
...
impl CompositeSigningKey {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let ed25519_sk: Ed25519SigningKey =
            SigningKeyTrait::from_bytes(&bytes[..32]).map_err(|e| {
        ...
        let ml_dsa_sk = MlDsa65SigningKey::from_bytes(&bytes[32..32 + 4032]).map_err(|e| {
            HoloCryptError::InvalidConfiguration {
                reason: format!("Invalid ML-DSA-65 signing key: {}", e),
            }
        })?;

        let ml_dsa_vk = MlDsa65VerifyingKey::from_bytes(&bytes[32 + 4032..]).map_err(|e| {
            HoloCryptError::InvalidConfiguration {
...
impl CompositeVerifyingKey {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let ed25519: Ed25519VerifyingKey =
            VerifyingKeyTrait::from_bytes(&bytes[..32]).map_err(|e| {
                HoloCryptError::InvalidConfiguration {
                    reason: format!("Invalid Ed25519 verifying key: {}", e),
                }
            })?;

        let ml_dsa = MlDsa65VerifyingKey::from_bytes(&bytes[32..]).map_err(|e| {
            HoloCryptError::InvalidConfiguration {
                reason: format!("Invalid ML-DSA-65 verifying key: {}", e),
            }
        })?;
    ...
    pub fn verify(&self, message: &[u8], signature: &CompositeSignature) -> Result<()> {
        ...
        VerifyingKeyTrait::verify(&self.ed25519, message, &signature.ed25519).map_err(|_| {
            HoloCryptError::CompositeSignatureInvalid {
                component: "Ed25519".to_string(),
            }
        })?;

        // Verify ML-DSA-65 signature
        MlDsa65::verify(&self.ml_dsa, message, &signature.ml_dsa).map_err(|_| {
            HoloCryptError::CompositeSignatureInvalid {
...
impl CompositeSignature {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let ed25519 = SignatureTrait::from_bytes(&bytes[..64]).map_err(|_| {
            HoloCryptError::CompositeSignatureInvalid {
                component: "Ed25519 signature bytes".to_string(),
            }
        })?;

        let ml_dsa = MlDsa65Signature::from_bytes(&bytes[64..]).map_err(|_| {
            HoloCryptError::CompositeSignatureInvalid {
...
mod tests {
    ...
    fn test_individual_signature_verification() {
        ...
        assert!(MlDsa65::verify(vk.ml_dsa_key(), message, signature.ml_dsa_signature()).is_ok());
    }
}
