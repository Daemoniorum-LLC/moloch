...
pub use arcanum_agile::{AlgorithmId, Policy, ComplianceProfile, SecurityLevel};
...
impl AgileEncryptedEvent {
    ...
    pub fn unseal(&self, key: &[u8]) -> Result<Vec<u8>> {
        let plaintext = self.container.decrypt(key).map_err(|e| {
    ...
    pub fn migration_recommendation(&self) -> Option<MigrationInfo> {
        self.container.migration_recommendation().map(|r| MigrationInfo {
            source: r.source,
            target: r.target,
            reason: r.reason,
        })
