...
use arcanum_zkp::{DiscreteLogProof, RangeProof as BulletproofRangeProof};
        ...
        match &self.proof_type {
            ...
            ProofType::EqualityProof {
                ...
                Err(HoloCryptError::ZkProofInvalid {
                    reason: "Equality proofs not yet implemented - awaiting arcanum-zkp support"
                        .to_string(),
                })
            }
            _ => {
...
impl BulletproofRangeData {
    ...
    pub fn prove(value: u64, n_bits: usize) -> Result<Self> {
        let proof = BulletproofRangeProof::prove(value, n_bits).map_err(|e| {
    ...
    pub fn verify(&self) -> Result<bool> {
        let proof =
            BulletproofRangeProof::from_bytes(&self.proof_bytes, self.n_bits).map_err(|e| {
                HoloCryptError::ZkProofInvalid {
...
impl SchnorrProofData {
    ...
    /// Verify the Schnorr proof.
    pub fn verify(&self) -> Result<bool> {
        let proof = DiscreteLogProof::from_bytes(&self.proof_bytes).map_err(|e| {
            HoloCryptError::ZkProofInvalid {
        ...
        let public_key_compressed = arcanum_zkp::curve::CompressedRistretto::from_slice(
            &self.public_key,
        )
