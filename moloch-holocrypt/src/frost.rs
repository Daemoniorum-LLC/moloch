...
use arcanum_threshold::frost::{
    FrostSigner, FrostVerifier, GroupVerifyingKey, PublicKeyPackage,
    SigningCommitments, SigningNonces, SigningPackage, SignatureShare,
    Signature, trusted_dealer_keygen,
};
...
impl FrostCoordinator {
    ...
    pub fn setup(config: &FrostConfig) -> Result<(Self, Vec<FrostParticipant>)> {
        ...
        let group_key = GroupVerifyingKey::from_frost(pubkey_package.verifying_key())
            .map_err(|e| HoloCryptError::InvalidConfiguration {
    ...
    pub fn create_signing_package(
        ...
        let commitments: Vec<_> = round1_outputs.iter()
    ...
    pub fn aggregate(
        ...
        let verifier = FrostVerifier::new(&self.group_key)
            .map_err(|e| HoloCryptError::FrostRoundFailed {
                round: 2,
                reason: format!("Failed to create verifier: {}", e),
            })?;

        let signature = verifier.aggregate(signing_package, shares, &self.pubkey_package)
            .map_err(|e| HoloCryptError::FrostRoundFailed {
                round: 2,
    ...
    pub fn verify(&self, message: &[u8], signature: &FrostSignature) -> Result<bool> {
        ...
        verifier.verify(message, &signature.inner)
...
impl FrostParticipant {
    ...
    pub fn round1(&self) -> Result<Round1Output> {
        let (nonces, commitments) = self.signer.round1()
            .map_err(|e| HoloCryptError::FrostRoundFailed {
                round: 1,
    ...
    pub fn round2(
        ...
        self.signer.round2(message, nonces, signing_package)
            .map_err(|e| HoloCryptError::FrostRoundFailed {
                round: 2,
...
impl FrostSignature {
    ...
    pub fn verify(&self, message: &[u8]) -> Result<bool> {
        ...
        verifier.verify(message, &self.inner)
            .map_err(|_| HoloCryptError::SignatureInvalid)
    }
}
...
impl FrostSigningCeremony {
    ...
    pub fn has_round1_quorum(&self) -> bool {
        self.coordinator.config.has_quorum(self.round1_outputs.len())
    }
    ...
    pub fn create_signing_package(&mut self) -> Result<&SigningPackage> {
        ...
        let package = self.coordinator.create_signing_package(&outputs, &self.message)?;
    ...
    pub fn finalize(&mut self) -> Result<FrostSignature> {
        ...
        let signing_package = self.signing_package.as_ref()
...
mod tests {
    ...
    fn test_frost_signing_low_level() {
        ...
        let shares: Vec<_> = round1_outputs
            ...
            .map(|(r1, p)| {
                p.round2(message, &r1.nonces, &signing_package).unwrap()
            })
            .collect();
