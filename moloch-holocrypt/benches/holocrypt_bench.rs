...
use moloch_holocrypt::encrypted::{
    ...
    generate_keypair, EventSealingKey, KeyManager,
};
use moloch_holocrypt::proofs::{
    EventProof, BulletproofRangeData, SchnorrProofData, ProofVerifier, ProofAggregator,
};
use moloch_holocrypt::threshold::{
    ThresholdConfig, ThresholdEvent, KeyShare, KeyShareSet, ShareDistributor, ShareRefresher,
};
use moloch_holocrypt::pqc::{
    EventPqcKeyPair, PqcEvent, QuantumSafeEvent, KeyMigration,
};
use moloch_holocrypt::composite::{
    CompositeSigningKey, CompositeVerifyingKey, CompositeSignature,
};
use moloch_holocrypt::agile::{
    AgileConfig, AgileEncryptedEvent, Policy,
};
use moloch_holocrypt::frost::{
    FrostConfig, FrostCoordinator, FrostSignedEvent,
};

use moloch_core::crypto::SecretKey;
use moloch_core::event::{AuditEvent, ActorId, ActorKind, ResourceId, ResourceKind, EventType, Outcome};
...
fn bench_encrypted_events(c: &mut Criterion) {
    ...
    group.bench_function("keypair_generation", |b| {
        b.iter(|| {
            black_box(generate_keypair("bench-key"))
        })
    });
    ...
    group.bench_function("unseal", |b| {
        b.iter(|| {
            black_box(&encrypted).decrypt(black_box(&opening_key)).unwrap()
        })
    });

    group.bench_function("verify_structure", |b| {
        b.iter(|| {
            black_box(&encrypted).verify_structure(black_box(&opening_key)).unwrap()
        })
    });
    ...
    group.bench_function("id", |b| {
        b.iter(|| {
            black_box(&encrypted).id()
        })
    });

    group.bench_function("commitment", |b| {
        b.iter(|| {
            black_box(&encrypted).commitment()
        })
    });

    group.bench_function("merkle_root", |b| {
        b.iter(|| {
            black_box(&encrypted).merkle_root()
        })
    });

    group.bench_function("public_fields", |b| {
        b.iter(|| {
            black_box(&encrypted).public_fields()
        })
    });

    group.bench_function("is_field_public", |b| {
        b.iter(|| {
            black_box(&encrypted).is_field_public("event_type")
        })
    });
    ...
    group.bench_function("from_bytes", |b| {
        b.iter(|| {
            EncryptedEvent::from_bytes(black_box(&bytes)).unwrap()
        })
    });
    ...
    group.bench_function("key_manager_get_key", |b| {
        b.iter(|| {
            black_box(&manager).get_key(black_box("bench-key"))
        })
    });

    group.bench_function("key_manager_decrypt", |b| {
        b.iter(|| {
            black_box(&manager).decrypt(black_box(&encrypted)).unwrap()
        })
    });
...
fn bench_zkp(c: &mut Criterion) {
    ...
    for n_bits in [8, 16, 32, 64] {
        group.bench_with_input(
            ...
            |b, &n_bits| {
                b.iter(|| {
                    BulletproofRangeData::prove(black_box(42), black_box(n_bits)).unwrap()
                })
            },
        );
    }
    ...
    group.bench_function("schnorr_prove", |b| {
        b.iter(|| {
            SchnorrProofData::prove(black_box(&secret)).unwrap()
        })
    });
    ...
    group.bench_function("schnorr_verify", |b| {
        b.iter(|| {
            black_box(&schnorr_proof).verify().unwrap()
        })
    });
    ...
    group.bench_function("event_proof_verify", |b| {
        b.iter(|| {
            black_box(&existence_proof).verify(black_box(&encrypted)).unwrap()
        })
    });
    ...
    group.bench_function("event_proof_to_bytes", |b| {
        b.iter(|| {
            black_box(&existence_proof).to_bytes()
        })
    });
    ...
    group.bench_function("event_proof_from_bytes", |b| {
        b.iter(|| {
            EventProof::from_bytes(black_box(&proof_bytes)).unwrap()
        })
    });
    ...
    group.bench_function("proof_verifier_verify", |b| {
        b.iter(|| {
            black_box(&verifier).verify(black_box(&existence_proof), black_box(&encrypted)).unwrap()
        })
    });
    ...
    group.bench_function("proof_verifier_verify_all", |b| {
        b.iter(|| {
            black_box(&verifier).verify_all(black_box(&proofs), black_box(&encrypted)).unwrap()
        })
    });
    ...
    group.bench_function("proof_aggregator_aggregate", |b| {
        b.iter_batched(
            ...
            |agg| {
                agg.aggregate(black_box(&encrypted)).unwrap()
            },
...
fn bench_threshold(c: &mut Criterion) {
    ...
    group.bench_function("config_new", |b| {
        b.iter(|| {
            ThresholdConfig::new(black_box(2), black_box(3)).unwrap()
        })
    });

    group.bench_function("config_two_of_three", |b| {
        b.iter(|| {
            ThresholdConfig::two_of_three()
        })
    });

    group.bench_function("config_three_of_five", |b| {
        b.iter(|| {
            ThresholdConfig::three_of_five()
        })
    });

    group.bench_function("config_five_of_seven", |b| {
        b.iter(|| {
            ThresholdConfig::five_of_seven()
        })
    });
    ...
    for (name, config) in &configs {
        group.bench_function(format!("seal_{}", name), |b| {
            b.iter(|| {
                ThresholdEvent::seal(black_box(&event), config.clone()).unwrap()
            })
        });
    }
    ...
    for (name, config) in &configs {
        ...
        group.bench_function(format!("unseal_{}", name), |b| {
            b.iter(|| {
                black_box(&threshold_event).unseal(black_box(&share_set)).unwrap()
            })
        });
    }
    ...
    group.bench_function("verify_structure", |b| {
        b.iter(|| {
            black_box(&threshold_event).verify_structure().unwrap()
        })
    });
    ...
    group.bench_function("threshold_event_to_bytes", |b| {
        b.iter(|| {
            black_box(&threshold_event).to_bytes()
        })
    });
    ...
    group.bench_function("threshold_event_from_bytes", |b| {
        b.iter(|| {
            ThresholdEvent::from_bytes(black_box(&te_bytes)).unwrap()
        })
    });
    ...
    group.bench_function("key_share_from_bytes", |b| {
        b.iter(|| {
            KeyShare::from_bytes(black_box(&share_bytes)).unwrap()
        })
    });
    ...
    group.bench_function("key_share_set_has_threshold", |b| {
        b.iter(|| {
            black_box(&share_set).has_threshold()
        })
    });
    ...
    group.bench_function("share_refresh", |b| {
        b.iter(|| {
            ShareRefresher::refresh(black_box(&share_set), black_box(&config)).unwrap()
        })
    });
    ...
    group.bench_function("share_distributor_collect", |b| {
        b.iter(|| {
            black_box(&distributor).collect(black_box(["alice", "bob"]))
        })
    });

    group.bench_function("share_distributor_get_shares", |b| {
        b.iter(|| {
            black_box(&distributor).get_shares(black_box("alice"))
        })
    });
...
fn bench_pqc(c: &mut Criterion) {
    ...
    group.bench_function("keypair_generation", |b| {
        b.iter(|| {
            EventPqcKeyPair::generate(black_box("bench-key"))
        })
    });
    ...
    group.bench_function("public_key_bytes", |b| {
        b.iter(|| {
            black_box(&pqc_key).public_key_bytes()
        })
    });

    group.bench_function("private_key_bytes", |b| {
        b.iter(|| {
            black_box(&pqc_key).private_key_bytes()
        })
    });
    ...
    group.bench_function("pqc_event_seal", |b| {
        b.iter(|| {
            PqcEvent::seal(black_box(&event), black_box(&pqc_key)).unwrap()
        })
    });
    ...
    group.bench_function("pqc_event_unseal", |b| {
        b.iter(|| {
            black_box(&pqc_event).unseal(black_box(&pqc_key)).unwrap()
        })
    });

    group.bench_function("pqc_event_verify_structure", |b| {
        b.iter(|| {
            black_box(&pqc_event).verify_structure().unwrap()
        })
    });
    ...
    group.bench_function("pqc_event_commitment", |b| {
        b.iter(|| {
            black_box(&pqc_event).commitment()
        })
    });

    group.bench_function("pqc_event_merkle_root", |b| {
        b.iter(|| {
            black_box(&pqc_event).merkle_root()
        })
    });
    ...
    group.bench_function("pqc_event_from_bytes", |b| {
        b.iter(|| {
            PqcEvent::from_bytes(black_box(&bytes)).unwrap()
        })
    });
    ...
    group.bench_function("quantum_safe_seal", |b| {
        b.iter(|| {
            QuantumSafeEvent::seal(black_box(&event), black_box(&pqc_key)).unwrap()
        })
    });
    ...
    group.bench_function("quantum_safe_unseal", |b| {
        b.iter(|| {
            black_box(&qs_event).unseal(black_box(&pqc_key)).unwrap()
        })
    });
    ...
    group.bench_function("quantum_safe_from_bytes", |b| {
        b.iter(|| {
            QuantumSafeEvent::from_bytes(black_box(&qs_bytes)).unwrap()
        })
    });
...
fn bench_composite(c: &mut Criterion) {
    ...
    for (name, message) in &messages {
        ...
        group.bench_with_input(
            BenchmarkId::new("sign", name),
            message,
            |b, msg| {
                b.iter(|| {
                    black_box(&signing_key).sign(black_box(msg))
                })
            },
        );
    }
    ...
    for (name, message) in &messages {
        ...
        group.bench_with_input(
            ...
            |b, (msg, sig)| {
                b.iter(|| {
                    black_box(&verifying_key).verify(black_box(msg), black_box(sig)).unwrap()
                })
            },
        );
    }
    ...
    group.bench_function("signing_key_to_bytes", |b| {
        b.iter(|| {
            black_box(&signing_key).to_bytes()
        })
    });
    ...
    group.bench_function("signing_key_from_bytes", |b| {
        b.iter(|| {
            CompositeSigningKey::from_bytes(black_box(&sk_bytes)).unwrap()
        })
    });
    ...
    group.bench_function("verifying_key_from_bytes", |b| {
        b.iter(|| {
            CompositeVerifyingKey::from_bytes(black_box(&vk_bytes)).unwrap()
        })
    });
    ...
    group.bench_function("verifying_key_to_hex", |b| {
        b.iter(|| {
            black_box(&verifying_key).to_hex()
        })
    });
    ...
    group.bench_function("verifying_key_from_hex", |b| {
        b.iter(|| {
            CompositeVerifyingKey::from_hex(black_box(&vk_hex)).unwrap()
        })
    });
    ...
    group.bench_function("signature_from_bytes", |b| {
        b.iter(|| {
            CompositeSignature::from_bytes(black_box(&sig_bytes)).unwrap()
        })
    });
    ...
    group.bench_function("ed25519_key_access", |b| {
        b.iter(|| {
            black_box(&verifying_key).ed25519_key()
        })
    });

    group.bench_function("ml_dsa_key_access", |b| {
        b.iter(|| {
            black_box(&verifying_key).ml_dsa_key()
        })
    });
...
fn bench_agile(c: &mut Criterion) {
    ...
    for (name, config) in &configs {
        ...
        group.bench_function(format!("unseal_{}", name), |b| {
            b.iter(|| {
                black_box(&event).unseal(black_box(&key)).unwrap()
            })
        });
    }
    ...
    group.bench_function("algorithm", |b| {
        b.iter(|| {
            black_box(&event).algorithm()
        })
    });

    group.bench_function("config", |b| {
        b.iter(|| {
            black_box(&event).config()
        })
    });

    group.bench_function("commitment", |b| {
        b.iter(|| {
            black_box(&event).commitment()
        })
    });

    group.bench_function("merkle_root", |b| {
        b.iter(|| {
            black_box(&event).merkle_root()
        })
    });

    group.bench_function("needs_migration", |b| {
        b.iter(|| {
            black_box(&event).needs_migration()
        })
    });

    group.bench_function("migration_recommendation", |b| {
        b.iter(|| {
            black_box(&event).migration_recommendation()
        })
    });
    ...
    group.bench_function("complies_with_fips", |b| {
        b.iter(|| {
            black_box(&event).complies_with(black_box(&fips_policy))
        })
    });
    ...
    group.bench_function("complies_with_default", |b| {
        b.iter(|| {
            black_box(&event).complies_with(black_box(&default_policy))
        })
    });
    ...
    group.bench_function("from_bytes", |b| {
        b.iter(|| {
            AgileEncryptedEvent::from_bytes(black_box(&bytes)).unwrap()
        })
    });
...
fn bench_frost(c: &mut Criterion) {
    ...
    group.bench_function("config_new", |b| {
        b.iter(|| {
            FrostConfig::new(black_box(2), black_box(3))
        })
    });
    ...
    for (name, config) in &configs {
        group.bench_function(format!("setup_{}", name), |b| {
            b.iter(|| {
                FrostCoordinator::setup(black_box(config)).unwrap()
            })
        });
    }
    ...
    group.bench_function("config_threshold", |b| {
        b.iter(|| {
            black_box(&config).threshold()
        })
    });

    group.bench_function("config_total", |b| {
        b.iter(|| {
            black_box(&config).total()
        })
    });

    group.bench_function("config_has_quorum", |b| {
        b.iter(|| {
            black_box(&config).has_quorum(black_box(2))
        })
    });
    ...
    group.bench_function("coordinator_group_key", |b| {
        b.iter(|| {
            black_box(&coordinator).group_key()
        })
    });

    group.bench_function("coordinator_config", |b| {
        b.iter(|| {
            black_box(&coordinator).config()
        })
    });
    ...
    group.bench_function("participant_id", |b| {
        b.iter(|| {
            black_box(&participants[0]).id()
        })
    });

    group.bench_function("round1", |b| {
        b.iter(|| {
            participants[0].round1().unwrap()
        })
    });
    ...
    group.bench_function("create_signing_package", |b| {
        b.iter_batched(
            ...
            |outputs| {
                coordinator.create_signing_package(black_box(&outputs), black_box(message)).unwrap()
            },
            criterion::BatchSize::SmallInput,
        )
    });
    ...
    group.bench_function("round2", |b| {
        b.iter_batched(
            || {
                let r1s: Vec<_> = participants.iter().take(2).map(|p| p.round1().unwrap()).collect();
                let pkg = coordinator.create_signing_package(&r1s, message).unwrap();
                (r1s, pkg)
            },
            |(r1s, pkg)| {
                participants[0].round2(
                    black_box(message),
    ...
    group.bench_function("aggregate", |b| {
        b.iter_batched(
            || {
                let r1s: Vec<_> = participants.iter().take(2).map(|p| p.round1().unwrap()).collect();
                let pkg = coordinator.create_signing_package(&r1s, message).unwrap();
                let shares: Vec<_> = r1s.iter().zip(participants.iter().take(2))
                    .map(|(r1, p)| p.round2(message, &r1.nonces, &pkg).unwrap())
                    .collect();
                (pkg, shares)
            },
            |(pkg, shares)| {
                coordinator.aggregate(black_box(&pkg), black_box(&shares)).unwrap()
            },
            criterion::BatchSize::SmallInput,
        )
    });
    ...
    group.bench_function("full_ceremony_2_of_3", |b| {
        b.iter_batched(
            ...
            |(coord, parts)| {
                ...
                let shares: Vec<_> = r1s.iter().zip(parts.iter().take(2))
                    .map(|(r1, p)| p.round2(message, &r1.nonces, &pkg).unwrap())
                    .collect();
    ...
    group.bench_function("signature_verify", |b| {
        b.iter(|| {
            black_box(&signature).verify(black_box(message)).unwrap()
        })
    });

    group.bench_function("coordinator_verify", |b| {
        b.iter(|| {
            black_box(&coordinator).verify(black_box(message), black_box(&signature)).unwrap()
        })
    });
    ...
    let event_r1s: Vec<_> = event_parts.iter().take(2).map(|p| p.round1().unwrap()).collect();
    let event_pkg = event_coord.create_signing_package(&event_r1s, &event_message).unwrap();
    let event_shares: Vec<_> = event_r1s.iter().zip(event_parts.iter().take(2))
    ...
    group.bench_function("frost_signed_event_verify_signature", |b| {
        b.iter(|| {
            black_box(&frost_event).verify_signature().unwrap()
        })
    });
